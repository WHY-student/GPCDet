...
# 这是一个例子
    ...
    def __init__(self,
        ...
        super(FPNFUSE, self).__init__(init_cfg)
        ...
        self.fuse=nn.Conv2d(out_channels,1,kernel_size=1,stride=1,bias=True)
    ...
    def forward(self, inputs): # 这里输入是backbone传来的4层的张量，只用到后三层
        ...
        for i in range(used_backbone_levels - 1, 0, -1):
            ...
            if 'scale_factor' in self.upsample_cfg:
                ...
                # 上采样
                laterals[i]=F.interpolate(laterals[i],**self.upsample_cfg)
                # 计算权重
                weigh=torch.sigmoid(self.fuse(laterals[i]))
                laterals[i - 1] =2*weigh*laterals[i]+2*(1-weigh)*laterals[i-1]
            else:
