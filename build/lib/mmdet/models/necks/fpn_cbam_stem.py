...
class FPN_CBAM_STEM(BaseModule):
    """
# 这是一个例子
    Example:
        >>> import torch
    ...
    def __init__(self,
        ...
        super(FPN_CBAM_STEM, self).__init__(init_cfg)
        ...
        self.cbma=CBAMLayer(out_channels)
    ...
    def forward(self, inputs): # 这里输入是backbone传来的4层的张量，只用到后三层
        ...
        laterals=[
            lateral(inputs[i + self.start_level])for i, lateral in enumerate(self.lateral_convs)
        ]
        for i in range(len(laterals)):
            laterals[i]=self.cbma(laterals[i])
