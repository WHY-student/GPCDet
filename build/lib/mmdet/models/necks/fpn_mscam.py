...
from mmdet.models.utils import MS_CAM

from ..builder import NECKS


@NECKS.register_module()
class FPN_MSCAM(BaseModule):
    """
# 这是一个例子
    Example:
        >>> import torch
    ...
    def __init__(self,
        ...
        super(FPN_MSCAM, self).__init__(init_cfg)
        ...
        self.mscam=MS_CAM(channels=out_channels)
    ...
    def forward(self, inputs): # 这里输入是backbone传来的4层的张量，只用到后三层
        ...
        for i in range(len(laterals)):
            laterals[i]=self.mscam(laterals[i])
        #  向上插值在相加
