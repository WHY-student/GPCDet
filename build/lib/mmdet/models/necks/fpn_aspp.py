...
from .rfp import ASPP
from ..builder import NECKS


@NECKS.register_module()
class FPN_ASPP(BaseModule):
    """
# 这是一个例子
    Example:
        >>> import torch
    ...
    def __init__(self,
        ...
        super(FPN_ASPP, self).__init__(init_cfg)
        ...
        for i in range(self.start_level, self.backbone_end_level): # start level从1开始到3
            l_conv = ConvModule(
                ...
                act_cfg=act_cfg,
                inplace=False)
            fpn_conv = ASPP(in_channels=out_channels,out_channels=out_channels//4)
    ...
    def forward(self, inputs): # 这里输入是backbone传来的4层的张量，只用到后三层
        ...
        elif self.up_sample==" PixelShuffle":
            for i in range(used_backbone_levels - 1, 0, -1):
                laterals[i - 1] = laterals[i - 1] + self.ps(laterals[i])
