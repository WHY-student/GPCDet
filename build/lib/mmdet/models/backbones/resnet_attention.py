...
from ..utils import NAMAttention,CoordAtt

class BasicBlock(BaseModule):
    expansion = 1  # 输出通道数为输入通道数的倍数.
    # conv_3×3 --> bn1 --> relu --> conv_3×3 --> bn2 --> relu
    def __init__(self,
                 inplanes,
                 planes,
                 stride=1,
    ...
    def forward(self, x):
        ...
        def _inner_forward(x):
            ...
            if self.downsample is not None: # 需要保证 identity 和 x 的宽高相同.
...
class ResNet_ATTENTION(BaseModule):
    ...
    def __init__(self,
                 ...
                 strides=(1, 2, 2, 2),  # 每个stage第一个残差块的stride参数
                 dilations=(1, 1, 1, 1), # 空洞卷积参数设置
                 out_indices=(0, 1, 2, 3), # 输出特征图的索引，每个stage对应一个
                 style='pytorch',
                 deep_stem=False,  # 是否用3个3×3的卷积核代替主干上1个7×7的卷积核
                 ...
                 stage_with_dcn=(False, False, False, False),  # 指定哪个stage使用dcn
                 plugins=None,
                 with_cp=False,
                 attention=None,
        ...
        for i, num_blocks in enumerate(self.stage_blocks):
            ...
            if self.attention_name=='CALayer':
                attention_=CALayer(self.inplanes)
                self.attentions.append(attention_)
            elif self.attention_name=='NAMAttention':
                attention_ = NAMAttention(self.inplanes,self.device,self.chann,self.spatial)
                self.attentions.append(attention_)
            elif self.attention_name=='CoordAttention':
                attention_=CoordAtt(self.inplanes,self.inplanes)
                self.attentions.append(attention_)
