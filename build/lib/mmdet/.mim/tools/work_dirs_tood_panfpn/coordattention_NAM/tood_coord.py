...
train_pipeline = [
    dict(type='LoadImageFromFile', aug=False, percent=1, light_limt=85),
...
data = dict(
    ...
    train=dict(
        ...
        pipeline=[
            dict(
                type='LoadImageFromFile', aug=False, percent=1, light_limt=85),
            dict(type='LoadAnnotations', with_bbox=True),
            dict(type='Resize', img_scale=(1333, 800), keep_ratio=True),
            dict(type='RandomFlip', flip_ratio=0.5),
            dict(
...
work_dir = 'work_dirs_tood_panfpn/coordattention_NAM'
auto_resume = False
gpu_ids = [0]
