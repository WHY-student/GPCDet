...
class Resize:
    """Resize images & bbox & mask.
        ...
        该变换将输入图像的大小调整到一定的比例。Bbox和掩码是
        然后用相同的比例因子调整大小。如果输入dict包含键“scale”，则使用输入dict中的scale，否则使用init方法中的scale。
        如果输入dict包含键“scale_factor”（如果MultiScaleFlipAug没有给出img_scale，但scale_factor），实际比例将通过图像形状和scale_factor。
