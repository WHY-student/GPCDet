        ...
        _adj = np.array(
            [[1.0, 0.715, 0.21, 0.122, 0.448, 0.092, 0.609, 0.083, 0.264, 0.295, 0.144, 0.04, 0.013, 0.026, 0.178,
             ...
             [0.44, 0.564, 0.171, 0.098, 0.349, 0.062, 0.309, 0.218, 0.225, 0.276, 0.138, 0.022, 0.007, 0.102, 0.415,
              1.0]])
        # _adj=np.array([[1.0,0.624,0.132,0.075,0.315,0.063,0.512,0.051,0.166,0.202,0.094,0.039,0.008,0.016,0.117,0.124],
        #                [0.159,1.0,0.102,0.041,0.265,0.049,0.16,0.04,0.064,0.075,0.026,0.019,0.01,0.018,0.09,0.05],
        #                [0.256,0.768,1.0,0.184,0.492,0.102,0.28,0.078,0.138,0.136,0.048,0.036,0.019,0.058,0.221,0.079],
        #                [0.324,0.699,0.411,1.0,0.505,0.167,0.408,0.174,0.154,0.14,0.07,0.03,0.043,0.11,0.294,0.104],
        #                [0.139,0.455,0.112,0.051,1.0,0.108,0.216,0.062,0.065,0.071,0.027,0.022,0.02,0.025,0.109,0.046],
        #                [0.22,0.669,0.183,0.134,0.852,1.0,0.323,0.102,0.097,0.083,0.035,0.04,0.067,0.035,0.215,0.062],
        #                [0.46,0.56,0.13,0.085,0.442,0.083,1.0,0.068,0.125,0.137,0.064,0.024,0.026,0.024,0.147,0.078],
        #                [0.06,0.185,0.047,0.047,0.167,0.034,0.089,1.0,0.026,0.278,0.012,0.014,0.025,0.032,0.254,0.224],
        #                [0.504,0.759,0.215,0.108,0.447,0.084,0.422,0.068,1.0,0.26,0.108,0.028,0.023,0.023,0.145,0.148],
        #                [0.184,0.266,0.064,0.03,0.147,0.022,0.14,0.216,0.078,1.0,0.09,0.033,0.001,0.006,0.324,0.25],
        #                [0.535,0.588,0.142,0.093,0.354,0.058,0.407,0.058,0.204,0.566,1.0,0.053,0.0,0.022,0.08,0.173],
        #                [0.35,0.678,0.168,0.063,0.455,0.105,0.238,0.105,0.084,0.329,0.084,1.0,0.007,0.0,0.245,0.175],
        #                [0.147,0.765,0.191,0.191,0.853,0.368,0.559,0.412,0.147,0.015,0.0,0.015,1.0,0.191,0.279,0.044],
        #                [0.164,0.711,0.305,0.258,0.586,0.102,0.273,0.273,0.078,0.062,0.039,0.0,0.102,1.0,0.695,0.25],
        #                [0.126,0.375,0.122,0.073,0.266,0.066,0.175,0.231,0.051,0.38,0.015,0.029,0.016,0.074,1.0,0.335],
        #                [0.152,0.239,0.05,0.029,0.128,0.022,0.107,0.233,0.059,0.335,0.037,0.024,0.003,0.03,0.382,1.0]])
...
class TOODNoneGCNHead(ATSSHead):
    ...
    def __init__(self,
        ...
        super(TOODNoneGCNHead, self).__init__(num_classes, in_channels, **kwargs)
    ...
    def _init_layers(self):
        ...
        self.gc_fc = nn.Linear(self.num_classes *2048 , self.num_classes*256)
        self.gc_fc2=nn.Linear(self.num_classes*256,256)
        ...
        self.adj=ADJ(num_classes=self.num_classes,t=self.t,w=self.w)
               # 暂时缺一个输入词向量待解决，先随机生成
        #self.in_feat = torch.rand(self.num_classes, 2048)
        # resnet50获取特征
        fea=ResnetFeature()
    ...
    def forward(self, feats):
        ...
        for idx, (x, scale, stride) in enumerate(
            ...
            #直接使用特征
            g_feat =self.in_feat.to(cls_feat.device)
            g_feat_flattened = g_feat.view(-1)
            g_feat_flattened=self.gc_fc2(self.gc_fc(g_feat_flattened))
            g_feat_flattened = g_feat_flattened.unsqueeze(0).unsqueeze(-1).unsqueeze(-1)
            g_feat_flattened = g_feat_flattened.repeat(b, 1, h, w)
