...
class AnchorHead(BaseDenseHead, BBoxTestMixin):
    ...
    def __init__(self,
        ...
        self.use_sigmoid_cls = loss_cls.get('use_sigmoid', False)# 没传参则默认为false，
        if self.use_sigmoid_cls: # 没有背景类
            self.cls_out_channels = num_classes # 输出为 16
        else:
        ...
        if self.train_cfg:
            ...
            if hasattr(self.train_cfg,  # hasattr() 函数用于判断对象是否包含对应的属性。
        ...
        # list[int]: The number of priors (anchors) at a pointon the feature grid
        self.num_base_priors = self.prior_generator.num_base_priors[0]
        self._init_layers() # 如果子类继承了调用子类的
    ...
    def get_anchors(self, featmap_sizes, img_metas, device='cuda'):
        ...
        if self.rf_based == True:
            multi_level_anchors = self.anchor_generator.anchor2rf(
                multi_level_anchors, decay=self.decay, device=device)
