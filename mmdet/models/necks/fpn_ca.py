...
# 这是一个例子
    ...
    def __init__(self,
        ...
        super(FPN_CA, self).__init__(init_cfg)
       ...
       # self.fusionFactor = nn.ModuleList()
        for i in range(self.start_level, self.backbone_end_level): # start level从1开始到3
            if self.ca:
                ...
                l_conv=NAM(in_channels[i],out_channels)
            elif self.namca:
                l_conv=CoordNAM(in_channels[i],out_channels)
            else:
                l_conv = ConvModule(
                    in_channels[i],
                    out_channels,
    ...
    def forward(self, inputs): # 这里输入是backbone传来的4层的张量，只用到后三层
        ...
        for i in range(used_backbone_levels - 1, 0, -1):
            ...
            if 'scale_factor' in self.upsample_cfg:
                ...
                laterals[i - 1] = laterals[i - 1] + self.fusion[i-1] * F.interpolate(
