...
from ..utils.attention import CBAM,CBAMSPA,GAMAttention,GAMSPAAttention,BAM,BAMSPA,GCNet,CoordAtt
...
# 这是一个例子
    ...
    def __init__(self,
        ...
        self.fusionFactor=nn.ModuleList()
        for i in range(self.start_level, self.backbone_end_level):  # start level从1开始到3
            l_conv = CBAM(in_channels[i],out_channels)
            #l_conv = CBAMSPA(in_channels[i], out_channels)
            #l_conv=GAMAttention(in_channels[i],out_channels)
            #l_conv=GAMSPAAttention(in_channels[i],out_channels)
            #l_conv=BAM(in_channels[i],out_channels)
            #l_conv=BAMSPA(in_channels[i],out_channels)
            #l_conv=GCNet(c1=in_channels[i],inplanes=out_channels)
            ...
            # factor = nn.Sequential(nn.AdaptiveAvgPool2d((1, 1)),
            #                        nn.Flatten(),
            #                        nn.Linear(out_channels, 1))
            #self.fusionFactor.append(factor)
    ...
    def forward(self, inputs): # 这里输入是backbone传来的4层的张量，只用到后三层
        ...
        for i in range(used_backbone_levels - 1, 0, -1):
            ...
            fusion = 1
            # if self.fusion:
            #     fusion = torch.sigmoid(self.fusionFactor[i](laterals[i]).view(-1)).view(-1, 1, 1, 1)
            if 'scale_factor' in self.upsample_cfg:
                # fix runtime error of "+=" inplace operation in PyTorch 1.10
                laterals[i - 1] = laterals[i - 1] + fusion * F.interpolate(
