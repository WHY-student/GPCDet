...
from ..utils.attention import CoordAtt,CoordNAM,NAM
@NECKS.register_module()
class FPN_CA_FACTORATT(BaseModule):
    """
# 这是一个例子
    Example:
        >>> import torch
    ...
    def __init__(self,
        ...
        super(FPN_CA_FACTORATT, self).__init__(init_cfg)
        ...
        for i in range(self.start_level, self.backbone_end_level): # start level从1开始到3
            if self.ca:
                l_conv=CoordAtt(in_channels[i],out_channels)
            else:
                l_conv = ConvModule(
                    in_channels[i],
                    out_channels,
    ...
    def forward(self, inputs): # 这里输入是backbone传来的4层的张量，只用到后三层
        ...
        for i in range(used_backbone_levels - 1, 0, -1):
            ...
            factor = self.fusionFactor[i - 1](laterals[i].detach())
            #(n,1)
            #factor=(factor+1)/2
            #factor = factor.unsqueeze(-1).unsqueeze(-1) #(n,1,1,1)
            #factor=torch.sigmoid(factor)/2
            # 对批次维度进行平均，改变维度为 (1, 1, 1, 1)
            #factor = torch.mean(factor, dim=0, keepdim=True)
            if 'scale_factor' in self.upsample_cfg:
                # fix runtime error of "+=" inplace operation in PyTorch 1.10
                laterals[i - 1] = laterals[i - 1] + factor * F.interpolate(
