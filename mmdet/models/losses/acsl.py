...
class ACSL(nn.Module):
    ...
    def forward(self,
        ...
        self.n_i, self.n_c = cls_logits.size()  # n_i anchor数量 n_c类别数
        ...
        labels=labels[0]
        class_nums=cls_logits.size(1)
        target=F.one_hot(labels, num_classes=class_nums + 1)
        target=target[:,:class_nums]
        labels = torch.masked_select(labels, labels != class_nums)



        unique_label = torch.unique(labels)#将x中的不重复元素挑了出来，并且默认为生序排列
        with torch.no_grad():
        ...
        for cls in unique_label:
            ...
            #tmp_label_vec = cls_logits.new_zeros(self.n_c)  tensor(c) 0张量
           # tmp_label_vec[cur_labels] = 1  # tensor 对应位置变为1
        ...
        loss = weight_reduce_loss(cls_loss, weight, self.reduction, avg_factor)

        #loss=torch.sum(cls_loss) / self.n_i
        return loss
...
# class ACSL(nn.Module):
#
#     def __init__(self, score_thr=0.7,use_sigmoid=True, loss_weight=1.0):
#
#         super(ACSL, self).__init__()
#
#         self.score_thr = score_thr
...
#         # expand the labels to all their parent nodes
#         target = cls_logits.new_zeros(self.n_i, self.n_c)  # tensor(n,c)
#         # weight mask, decide which class should be ignored
#         # weight_mask = cls_logits.new_zeros(self.n_i, self.n_c)
#
...
#         # this is also applied to negative samples
#         high_score_inds = torch.nonzero(sigmoid_cls_logits >= self.score_thr)
#         weight_mask = torch.sparse_coo_tensor(high_score_inds.t(), cls_logits.new_ones(high_score_inds.shape[0]),
