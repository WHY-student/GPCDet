...
class ResNet_SAC_ATTENTION(ResNet_ATTENTION):
    ...
    Args:
        ...
        rfp_inplanes (int, optional): The number of channels from RFP.
            Default: None. 如果指定，则将添加一个转换层为添加 ``rfp_feat``. Otherwise, the structure is the same as
            base class.
        output_img (bool): If ``True``, 输入图像将被插入输出的起始位置. Default: False.
    """
    ...
    def __init__(self,
        ...
        super(ResNet_SAC_ATTENTION, self).__init__(**kwargs)
    ...
    def forward(self, x):
        """Forward function."""
        outs = list(super(ResNet_SAC_ATTENTION, self).forward(x))
        if self.output_img:
            outs.insert(0, x)
        return tuple(outs)
