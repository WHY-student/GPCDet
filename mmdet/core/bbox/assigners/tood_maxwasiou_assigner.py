...
class ToodMaxWass(BaseAssigner):
    ...
    def __init__(self, topk,pos_iou_thr=0.3,div=4.0,iou_calculator=dict(type='BboxOverlaps2D'),iou_calculator2=dict(type='BboxDistanceMetric'),
                 assign_metric='iou',assign_metric2='wasserstein'):
        self.pos_iou_thr = pos_iou_thr
        assert topk >= 1
        self.topk = topk
        self.div=div
        self.iou_calculator = build_iou_calculator(iou_calculator)
        self.iou_calculator2=build_iou_calculator(iou_calculator2)
        self.assign_metric2=assign_metric2
    ...
    def assign(self,
        ...
        _, candidate_idxs = alignment_metrics.topk(topk, dim=0, largest=True)#tensor(topk,m) m是gt数量 candidate_idxs 是包含前 k 个anchor对应的索引的张量。
        #从 alignment_metrics 中选择具有最高分数的候选行，并与目标相对应的列，然后将它们存储在 candidate_metrics 张量中。
        # 维度为(topk,m)由索引变成了metric中原始的值
        ...
        # Check if any part of the 1/4 rectangle is inside the gt
        #is_in_gts = is_in_gts.any(dim=1)

        # result=(is_pos&~is_in_gts)|(~is_pos&is_in_gts)
        # indexMid = t[result.view(-1)]
        # assigned_gt_inds[indexMid] = -1
        is_in_gts=overlaps2>0
