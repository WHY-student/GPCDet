...
def job():
    os.system("CUDA_VISIBLE_DEVICES=0,3 python -m torch.distributed.launch --nproc_per_node=2 --master_port=29500 train.py --launcher pytorch")

# 安排任务每天的18:00执行
schedule.every().day.at("04:14").do(job)
...
#
# # 设置定时任务
# #schedule.every().day.at("01:10:00").do(run_script)  # 在第一个时间点运行脚本
# schedule.every().day.at("04:30:00").do(run_script)  # 在第二个时间点运行脚本
#
# # 循环执行定时任务
# print("start")
# while True:
