...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=1000, zero_init_residual=False,
        ...
        # 2048 -> num_classes
        # ----------------------------------------#
        self.fc0 = nn.Linear(512 * block.expansion,1024)
    ...
    def forward(self, x):
        ...
        tensor_list = x.view(-1).tolist()

        #保存列表到本地txt文件，使用空格分隔
        with open('/home/duomeitinrfx/users/pengxl/mmdetection/法器1024txt/16旗帜.txt', 'w') as file:
            file.write(" ".join(map(str, tensor_list)))
        x = self.fc(x)
        return x
